...
void init_mouse_cursor8(char *mouse, char bc);	//我靠！ 终于有鼠标了
...
void HariMain(void)
{
	...
	/*
		import ascll 我太难了 ○|￣|_ =3
		putfonts8_asc(binfo->vram, binfo->scrnx, 30, 30, COL8_000000, "hello,world.");	2019-09-25 22:00
	*/
	/* 显示鼠标 */
...
void boxfill8(unsigned char *vram, int xsize, unsigned char c, int x0, int y0, int x1, int y1)
{
	...
	for (y = y0; y <= y1; y++) {
		for (x = x0; x <= x1; x++)
			vram[y * xsize + x] = c;			//0xa0000 + x + y *320
	}
	return;
}
...
void putfont8(char* vram, int xsize, int x, int y, char c, char* font)
{
	int i;
	char* p, d													/* data */;
	for (i = 0; i < 16; i++) {									//通过“与”判断哪一位是1，16进制数简化代码
		p = vram + (y + i) * xsize + x;
		d = font[i];
		if ((d & 0x80) != 0) { p[0] = c; }
		if ((d & 0x40) != 0) { p[1] = c; }
...
{
	extern char hankaku[4096];									//导入字体库
	for (; *s != 0x00; s++)
	{
...
void init_mouse_cursor8(char* mouse, char bc)						//绘制一个16*16的鼠标
{
